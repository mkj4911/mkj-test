        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="post" action="{{ route('member.products.update', ['product' => $product->id])}}">
                    @csrf
                    @method('put')
                    <div class="-m-2">
                        ...
                        <div>
                            <x-select-image :images="$images" currentId="{{$product->image1}}" currentImage="{{$product->imageOne->filename ?? ''}}" name="image1" />
                            <x-select-image :images="$images" currentId="{{$product->image2}}" currentImage="{{$product->imageTwo->filename ?? ''}}" name="image2" />
                            <x-select-image :images="$images" currentId="{{$product->image3}}" currentImage="{{$product->imageThree->filename ?? ''}}" name="image3" />
                            <x-select-image :images="$images" currentId="{{$product->image4}}" currentImage="{{$product->imageFour->filename ?? ''}}" name="image4" />
                            <x-select-image :images="$images" currentId="{{$product->image5}}" currentImage="{{$product->imageFive->filename ?? ''}}" name="image5" />
                            <x-select-image :images="$images" currentId="{{$product->image6}}" currentImage="{{$product->imageSix->filename ?? ''}}" name="image6" />
                            <x-select-image :images="$images" currentId="{{$product->image7}}" currentImage="{{$product->imageSeven->filename ?? ''}}" name="image7" />
                            <x-select-image :images="$images" currentId="{{$product->image8}}" currentImage="{{$product->imageEight->filename ?? ''}}" name="image8" />
                            <x-select-image :images="$images" currentId="{{$product->image9}}" currentImage="{{$product->imageNine->filename ?? ''}}" name="image9" />
                    ...
                    <form id="delete_{{$product->id}}" method="post" action="{{ route('member.products.destroy', ['product' => $product->id]) }}">
                        @csrf
                        @method('delete')
                        <div class="p-2 w-full flex justify-around mt-16">
                            <button type="button" data-id="{{ $product->id }}" onclick="deletePost(this)" class="text-white bg-red-500 h-8 w-36 ring-8 ring-red-600 rounded-full hover:bg-red-300 text-lg">削除する</button>
                        </div>
                    </form>   
                </div>
            </div>
